...
- **Credentials**: `ANTHROPIC_API_KEY`, `CLAUDE_SDK_OAUTH_TOKEN`, or your Claude Code login is read from your environment by scuttlerun and pincenez, which prefer a Claude subscription over the API key by default (`--auth` / the `auth:` key in `evals.yaml` controls this; craboodle only forwards the chosen mode). An inherited `CLAUDE_CODE_OAUTH_TOKEN` is stripped by both tools and never used. Craboodle never reads or logs credentials.
