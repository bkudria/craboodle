...
- **Credentials**: `ANTHROPIC_API_KEY`, `CLAUDE_CODE_OAUTH_TOKEN`, or your Claude Code login is read from your environment by scuttlerun and pincenez, which prefer a Claude subscription over the API key by default (`--auth` / the `auth:` key in `evals.yaml` controls this; craboodle only forwards the chosen mode). Craboodle never reads or logs credentials.
