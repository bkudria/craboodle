...
- **Plugin-aware inputs.** In plugin mode (`.claude-plugin/plugin.json` present), craboodle parses the manifest and enumerates the plugin's components by filesystem convention (`skills/<id>/SKILL.md`, `agents/<id>.md`, `commands/<id>.md`, `hooks/hooks.json`, `.mcp.json`). The parsed manifest and component lists are surfaced through `EvalsConfig.plugin` so downstream features can identify, scaffold, or verify coverage without each one re-walking the tree.
- **Plugin-aware lint output.** In plugin mode, `craboodle lint` appends a `plugin_coverage:` block after its existing totals, reporting per-component scenario counts (zero-filled for declared components with no matching scenarios). Mapping is by name prefix: scenario `id` covers component `<type>/<id>` iff `id` equals `<type>-<id>` or starts with `<type>-<id>-`, longest-component-id-first on overlaps. The block is informational only — coverage gaps do not change exit codes, consistent with the raw-data principle.
